...
/*WHERE conditions are applied before the data enters the input buffer while IF conditions are applied */
/*after the data enters the program data vector. This is the reason why the WHERE condition is faster */
/*because not all observations have to be read and because it can only be applied on variables that exist */
...
/*•	Without the NWAY option in the PROC MEANS statement, the CLASS statement will calculate summaries for */
/*each class variable separately as well as for each possible combination of class variables. The BY statement */
/*only provides summaries for the groups created by the combination of all BY variables.*/
